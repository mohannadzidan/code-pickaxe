...
export const showDependentsOnlyCommand: Command = {
  id: 'showDependentsOnly',
  title: 'Show dependents only',
  description: 'Show only this node and the nodes that depend on it (incoming edges)',
  icon: ScanLine,
  keybinding: { key: 'd', modifiers: { shift: true } },

  predicate: (ctx) => {
    if (ctx.activeSurface !== 'graph' && ctx.activeSurface !== 'explorer') return false;
    if (!ctx.selectedEntityId) return false;
    const { edges } = useGraphStore.getState();
    return Object.values(edges).some((e) => e.target === ctx.selectedEntityId);
  },
